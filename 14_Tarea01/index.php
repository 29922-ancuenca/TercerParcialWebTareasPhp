<?php
// Usar la conexión centralizada y la clase producto
require_once 'conexion.php';
require_once 'producto.php';

// Si viene un POST desde el botón "Guardar en Base de Datos",
// insertar todas las filas en la tabla detalle_factura.
$mensaje_guardado = '';
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['nombre']) && is_array($_POST['nombre'])) {
    ...
    for ($i = 0; $i < count($nombres); $i++) {
        ...
        $total   = isset($totales[$i]) ? (float)$totales[$i] : ($precio * $cantidad);

        if ($precio <= 0 || $cantidad <= 0) {
            continue;
        }

        $sqlIns = "INSERT INTO detalle_factura (nombre, precio, cantidad, total) " .
    ...
    if ($insertados > 0) {
        $mensaje_guardado = "Se guardaron $insertados producto(s) en la base de datos.";
    } else {
        $mensaje_guardado = "No se pudo guardar la información en la base de datos.";
    }
}

$objProducto = new producto($con);
$opcionesProductos = $objProducto->get_select_options();
?>
<!doctype html>
<html lang="es">
<head>
...
<body>
  ...
  <div class="container" style="max-width: 850px;">
      ...
      <div class="main-card">
            ...
            <!-- Inputs -->
            <form id="formProductos" method="post" action="index.php">
                <div class="form-row">
        ...
        <div class="col-12 mt-4" id="editor" style="display:none;">
            <h5>Editar producto seleccionado</h5>
            <div class="table-responsive">
                <table class="table table-bordered text-center">
                    ...
                    <tbody>
                        <tr>
                            <td><input type="text" class="form-control" id="edit-nombre" readonly></td>
                            <td><input type="number" class="form-control" id="edit-precio" step="0.01" min="0" readonly></td>
                            <td><input type="number" class="form-control" id="edit-cantidad" min="1"></td>
                            <td><input type="text" class="form-control" id="edit-total" readonly></td>
                            <td>
                                <button type="button" class="btn btn-sm btn-success" id="edit-guardar">Guardar</button>
                                <button type="button" class="btn btn-sm btn-secondary" id="edit-cancelar">Cancelar</button>
                            </td>
                        </tr>
                    </tbody>
                </table>
